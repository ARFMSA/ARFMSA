...
class TitaNet(nn.Module):
    ...
    def get_titanet(
        ...
        attention_hidden_size=128,
        simple_pool=False,
        loss_function=losses.ArcFaceLoss(embedding_size=192,n_classes=251),
        dropout=0.5,
        device=torch.device('cuda'),
    ):
        """
    ...
    def forward(self, spectrograms, speakers=None):
       ...
       # embeddings = self.decoder(spectrograms)
        # Inference mode
        if speakers is None:
            return F.normalize(embeddings, p=2, dim=1)
...
def main():
    net=TitaNet.get_titanet().to(device)
    x=torch.randn(20,80,1024).to(device)
    out=net(x)
    print(out.shape)
